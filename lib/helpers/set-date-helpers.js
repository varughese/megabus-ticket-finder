...
function hasTodayKeyword(startDate) {
	return typeof startDate === "string" ? (startDate.toLowerCase().indexOf("today") > -1) : false;
}

function _getStartDate(_dateOpts) {
	var startDate = _dateOpts.start;
	if(!startDate || hasTodayKeyword(startDate))
		return moment();
	else
...
function _getDatesList(_dateOpts, _startDate, endDate) {
	...
	if(endDate.diff(_startDate, 'days') <= 7) _dateOpts.days = false;
